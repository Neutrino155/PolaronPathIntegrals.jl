...
function variation(α::Array, β::Array; v = 0.0, w = 0.0, ω = 1.0, N = 1, T = 20.0) # N number of v and w params
	...
	solution = Optim.optimize(
		...
		initial,
		Fminbox(LBFGS()),
		Optim.Options(f_reltol = 1e-3, x_reltol = 1e-3, show_trace = true), # Set time limit for asymptotic convergence if needed.
	)
...
function variation(α::Array; v = 0.0, w = 0.0, ω = 1.0, N = 1, T = 20) # N number of v and w params
	...
	solution = Optim.optimize(
		...
		initial,
		Fminbox(LBFGS()),
		Optim.Options(f_reltol = 1e-3, x_reltol = 1e-3), # Set time limit for asymptotic convergence if needed.
	)
